  ...
  return (
    <>
      ...
      <div id="default-carousel" className={`relative w-full overflow-hidden rounded-md flex space-x-4 justify-center transition-slide duration-${animationDuration} ${isAnimating ? 'opacity-0' : 'opacity-100' }`}>       
        ...
        <button type="button" className="absolute top-1/2 left-80 z-30 flex items-center justify-center w-10 h-10 bg-black bg-opacity-70 hover:bg-opacity-90 rounded-full cursor-pointer group focus:outline-none transform -translate-y-1/2" onClick={handlePrev}>
          <MdSkipPrevious/>
        </button>

        <button type="button" className="absolute top-1/2 right-80 z-30 flex items-center justify-center w-10 h-10 bg-black bg-opacity-70 hover:bg-opacity-90 rounded-full cursor-pointer group focus:outline-none transform -translate-y-1/2" onClick={handleNext}>
          <MdSkipNext/>
        </button>
      </div>
    </>
